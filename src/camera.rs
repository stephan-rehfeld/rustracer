...
use crate::math::{NormalizableVector, Normal3, Point2, Point3, Vector2, Vector3};
    ...
    <T as Div>::Output: Div<Output=<T as Div>::Output> + Half + Mul<T, Output=T> + Mul<<T as Div>::Output, Output=<T as Div>::Output> + Sub<Output=<T as Div>::Output> + Copy,
{
    ...
    fn ray_for(&self, p: Point2<<T as Div>::Output>) -> ParametricLine<Point3<T>, Vector3<T>> {
        ...
        let o = self.e + T::one() * self.aspect_ratio * self.scale * x * self.u + T::one() * self.scale * y * self.v;

        ParametricLine::new(o, d)
    }
}

pub struct Perspective<T> where T: Div {
    e: Point3<T>,
    u: Normal3<<T as Div>::Output>,
    v: Normal3<<T as Div>::Output>,
    w: Normal3<<T as Div>::Output>,
    ...
    macro_rules! new_orthographic {
        ($type: ty, $name: ident) => {
            ...
            fn $name() {
                ...
                assert_eq!(orth.e, e);
                assert_eq!(orth.u, Vector3::new(-1 as $type, 0 as $type, 0 as $type).normalized());
                assert_eq!(orth.v, Vector3::new(0 as $type, -1 as $type, 0 as $type).normalized());
                assert_eq!(orth.w, Vector3::new(0 as $type, 0 as $type, 1 as $type).normalized());
