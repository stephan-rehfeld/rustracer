...
//    let triangle_geometry = Box::new(rustracer::RenderableGeometry::new(triangle, color::RGB::new(1.0, 1.0, 0.0)));
    ...
    for (x, y, pixel) in imgbuf.enumerate_pixels_mut() {
        ...
        let mut hits : Vec<(f64, rustracer::NormalType<f64>, color::RGB<f64>)> = geometries.iter().flat_map(|g| g.intersect(ray)).filter(|(t,_,_)| *t > 0.0).collect();
